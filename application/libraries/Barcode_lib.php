...
class Barcode_lib
{
	...
	{
		//customized barcode image...
		$display_table="<table id='barcode_img'>";
		$display_table.='<tr><td align="center">
					<h2>'.$this->CI->config->item('company').'</h2>
					<h5><b>Article : '.$item['name'].'</b></h5>
					<h5><b>Color : '.$item['custom2'].'</b></h5>
					</td></tr>';
		$display_table.='<tr class="col-md-10"><td class="col-md-8"></td><td class="col-md-1"><img src="'.site_url('items/barcode_pic_thumb/' . pathinfo($item['pic_filename'], PATHINFO_BASENAME)) . '"></td><td align="left"><h1 class="h1">'.$item['custom1'].'</h1></td></tr>';
